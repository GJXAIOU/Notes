...
- 
  ...
  **
  ...
  二是对各区域的管理，假设这是一个全球性的系统，所以所有分享的空间以及申请存储服务应该以区域为划分，那按什么规则去划分区域，是否是要跨区域备份数据，如何检查区域内磁盘、DataNode等是否正常运行。
  三是如何处理DataNode和NameNode之间的通信。由于个人用户可能存在断网的情况，这样会阻碍到DataNode进行心跳，导致失效认证认证出错。
...
- 
  ...
  **
  ...
  老师请问下hdfs一般集群规模都会上百吗？那像我们小公司是不是没有意义啦，我们现在每天日志量100g左右，适合用hdfs来存储吗？单机采购什么规格的合适（几核几g多少t存储）？
...
- 
  ...
  **
  ...
  看了您讲的这节，感觉分布式其实就是单机技术的横向扩展，现在流行的Tidb的底层的分布式存储的原理跟hdfs的底层存储方式感觉挺像，其实灵感都来源于RAID的磁盘存储原理。
