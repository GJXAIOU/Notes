...
> [合并](https://clickhouse.com/docs/zh/engines/table-engines/special/merge#merge) 引擎并不属于 `*MergeTree` 系列。
...
- `SETTINGS` — 控制 `MergeTree` 行为的额外参数，可选项：
  ...
  - ```
    ...
    — 使用直接 I/O 来操作磁盘的合并操作时要求的最小数据量。合并数据片段时，ClickHouse 会计算要被合并的所有数据的总存储空间。如果大小超过了
    ...
    设置的字节数，则 ClickHouse 将使用直接 I/O 接口（
...
## 主键和索引在查询中的表现[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#primary-keys-and-indexes-in-queries)
...
### 主键的选择[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#zhu-jian-de-xuan-ze)
...
- 在[CollapsingMergeTree](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/collapsingmergetree#table_engine-collapsingmergetree) 和 [SummingMergeTree](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/summingmergetree) 引擎里进行数据合并时会提供额外的处理逻辑。

  在这种情况下，指定与主键不同的 *排序键* 也是有意义的。

长的主键会对插入性能和内存消耗有负面影响，但主键中额外的列并不影响 `SELECT` 查询的性能。

可以使用 `ORDER BY tuple()` 语法创建没有主键的表。在这种情况下 ClickHouse 根据数据插入的顺序存储。如果在使用 `INSERT ... SELECT` 时希望保持数据的排序，请设置 [max_insert_threads = 1](https://clickhouse.com/docs/zh/operations/settings/settings#settings-max-insert-threads)。

想要根据初始顺序进行数据查询，使用 [单线程查询](https://clickhouse.com/docs/zh/operations/settings/settings#settings-max_threads)

### 选择与排序键不同的主键[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#choosing-a-primary-key-that-differs-from-the-sorting-key)

Clickhouse可以做到指定一个跟排序键不一样的主键，此时排序键用于在数据片段中进行排序，主键用于在索引文件中进行标记的写入。这种情况下，主键表达式元组必须是排序键表达式元组的前缀(即主键为(a,b)，排序列必须为(a,b,***\***))。

当使用 [SummingMergeTree](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/summingmergetree) 和 [AggregatingMergeTree](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/aggregatingmergetree) 引擎时，这个特性非常有用。通常在使用这类引擎时，表里的列分两种：*维度* 和 *度量* 。典型的查询会通过任意的 `GROUP BY` 对度量列进行聚合并通过维度列进行过滤。由于 SummingMergeTree 和 AggregatingMergeTree 会对排序键相同的行进行聚合，所以把所有的维度放进排序键是很自然的做法。但这将导致排序键中包含大量的列，并且排序键会伴随着新添加的维度不断的更新。

在这种情况下合理的做法是，只保留少量的列在主键当中用于提升扫描效率，将维度列添加到排序键中。

对排序键进行 [ALTER](https://clickhouse.com/docs/zh/sql-reference/statements/alter) 是轻量级的操作，因为当一个新列同时被加入到表里和排序键里时，已存在的数据片段并不需要修改。由于旧的排序键是新排序键的前缀，并且新添加的列中没有数据，因此在表修改时的数据对于新旧的排序键来说都是有序的。

### 索引和分区在查询中的应用[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#use-of-indexes-and-partitions-in-queries)
...
要检查 ClickHouse 执行一个查询时能否使用索引，可设置 [force_index_by_date](https://clickhouse.com/docs/zh/operations/settings/settings#settings-force_index_by_date) 和 [force_primary_key](https://clickhouse.com/docs/zh/operations/settings/settings) 。
...
### 部分单调主键的使用[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#部分单调主键的使用)
...
`*MergeTree` 系列的表可以指定跳数索引。 跳数索引是指数据片段按照粒度(建表时指定的`index_granularity`)分割成小块后，将上述SQL的granularity_value数量的小块组合成一个大的块，对这些大块写入索引信息，这样有助于使用`where`筛选时跳过大量不必要的数据，减少`SELECT`需要读取的数据量。
...
#### 可用的索引类型[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-data_skipping-indexes)
...
- `ngrambf_v1(n, size_of_bloom_filter_in_bytes, number_of_hash_functions, random_seed)` 存储一个包含数据块中所有 n元短语（ngram） 的 [布隆过滤器](https://en.wikipedia.org/wiki/Bloom_filter) 。只可用在字符串上。 可用于优化 `equals` ， `like` 和 `in` 表达式的性能。
...
- `bloom_filter(bloom_filter([false_positive])` – 为指定的列存储布隆过滤器
  ...
  以下函数会用到这个索引： [equals](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions), [notEquals](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions), [in](https://clickhouse.com/docs/zh/sql-reference/functions/in-functions), [notIn](https://clickhouse.com/docs/zh/sql-reference/functions/in-functions), [has](https://clickhouse.com/docs/zh/sql-reference/functions/array-functions)
...
#### 函数支持[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#functions-support)
...
| ------------------------------------------------------------ | ----------- | ------ | ---------- | ---------- | ------------ |
| [equals (=, ==)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-equals) | ✔           | ✔      | ✔          | ✔          | ✔            |
| [notEquals(!=, <>)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-notequals) | ✔           | ✔      | ✔          | ✔          | ✔            |
| [like](https://clickhouse.com/docs/zh/sql-reference/functions/string-search-functions#function-like) | ✔           | ✔      | ✔          | ✔          | ✔            |
| [notLike](https://clickhouse.com/docs/zh/sql-reference/functions/string-search-functions#function-notlike) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [startsWith](https://clickhouse.com/docs/zh/sql-reference/functions/string-functions#startswith) | ✔           | ✔      | ✔          | ✔          | ✗            |
| [endsWith](https://clickhouse.com/docs/zh/sql-reference/functions/string-functions#endswith) | ✗           | ✗      | ✔          | ✔          | ✗            |
| [multiSearchAny](https://clickhouse.com/docs/zh/sql-reference/functions/string-search-functions#function-multisearchany) | ✗           | ✗      | ✔          | ✗          | ✗            |
| [in](https://clickhouse.com/docs/zh/sql-reference/functions/in-functions#in-functions) | ✔           | ✔      | ✔          | ✔          | ✔            |
| [notIn](https://clickhouse.com/docs/zh/sql-reference/functions/in-functions#in-functions) | ✔           | ✔      | ✔          | ✔          | ✔            |
| [less (\<)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-less) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [greater (>)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-greater) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [lessOrEquals (\<=)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-lessorequals) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [greaterOrEquals (>=)](https://clickhouse.com/docs/zh/sql-reference/functions/comparison-functions#function-greaterorequals) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [empty](https://clickhouse.com/docs/zh/sql-reference/functions/array-functions#function-empty) | ✔           | ✔      | ✗          | ✗          | ✗            |
| [notEmpty](https://clickhouse.com/docs/zh/sql-reference/functions/array-functions#function-notempty) | ✔           | ✔      | ✗          | ✗          | ✗            |
| hasToken                                                     | ✗           | ✗      | ✗          | ✔          | ✗            |
...
## 并发数据访问[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#concurrent-data-access)
...
要定义`interval`, 需要使用 [时间间隔](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#operators-datetime) 操作符。
...
### 列 TTL[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#mergetree-column-ttl)
...
### 表 TTL[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#mergetree-table-ttl)
...
### 介绍[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#introduction)
...
数据片段是 `MergeTree` 引擎表的最小可移动单元。属于同一个数据片段的数据被存储在同一块磁盘上。数据片段会在后台自动的在磁盘间移动，也可以通过 [ALTER](https://clickhouse.com/docs/zh/sql-reference/statements/alter#alter_move-partition) 查询来移动。

### 术语[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#terms)

- 磁盘 — 挂载到文件系统的块设备

- 默认磁盘 — 在服务器设置中通过 [path](https://clickhouse.com/docs/zh/operations/server-configuration-parameters/settings#server_configuration_parameters-path) 参数指定的数据存储
...
### 配置[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-multiple-volumes_configure)
...
可以通过 [background_move_pool_size](https://clickhouse.com/docs/zh/operations/server-configuration-parameters/settings#background_move_pool_size) 设置调整执行后台任务的线程数。

### 详细说明[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#details)

对于 `MergeTree` 表，数据通过以下不同的方式写入到磁盘当中：

- 插入（`INSERT`查询）
- 后台合并和[数据变异](https://clickhouse.com/docs/zh/sql-reference/statements/alter#alter-mutations)
- 从另一个副本下载
- [ALTER TABLE … FREEZE PARTITION](https://clickhouse.com/docs/zh/sql-reference/statements/alter#alter_freeze-partition) 冻结分区
...
在后台，数据片段基于剩余空间（`move_factor`参数）根据卷在配置文件中定义的顺序进行转移。数据永远不会从最后一个移出也不会从第一个移入。可以通过系统表 [system.part_log](https://clickhouse.com/docs/zh/operations/system-tables/part_log#system_tables-part-log) (字段 `type = MOVE_PART`) 和 [system.parts](https://clickhouse.com/docs/zh/operations/system-tables/parts#system_tables-parts) (字段 `path` 和 `disk`) 来监控后台的移动情况。具体细节可以通过服务器日志查看。

用户可以通过 [ALTER TABLE … MOVE PART|PARTITION … TO VOLUME|DISK …](https://clickhouse.com/docs/zh/sql-reference/statements/alter#alter_move-partition) 强制移动一个数据片段或分区到另外一个卷，所有后台移动的限制都会被考虑在内。这个查询会自行启动，无需等待后台操作完成。如果没有足够的可用空间或任何必须条件没有被满足，用户会收到报错信息。
...
## 使用S3进行数据存储[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#using-s3-data-storage)
...
## 虚拟列[](https://clickhouse.com/docs/zh/engines/table-engines/mergetree-family/mergetree#virtual-columns)
