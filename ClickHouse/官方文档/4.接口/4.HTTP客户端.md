...
如果你发送了一个未携带任何参数的`GET /`请求，它返回响应码200和`OK`字符串定义，可在[Http服务响应配置](https://clickhouse.com/docs/zh/operations/server-configuration-parameters/settings#server_configuration_parameters-http_server_default_response)定义(在末尾添加换行)
...
Web UI 可以通过这个地址访问: `http://localhost:8123/play`. 在运行状况检查脚本中，使用`GET /ping`请求。这个处理方法总是返回 "Ok"。(以换行结尾)。可从18.12.13版获得。请参见' /replicas_status '检查复制集的延迟。
...
默认情况下，返回的数据是`TabSeparated`格式的，更多信息，见[Formats](https://clickhouse.com/docs/zh/interfaces/formats)部分。
...
在传输数据时，可以使用ClickHouse内部压缩格式。压缩的数据具有非标准格式，您需要使用特殊的`clickhouse-compressor`程序来处理它(它是与`clickhouse-client`包一起安装的)。为了提高数据插入的效率，您可以通过使用[http_native_compression_disable_checksumming_on_decompress](https://clickhouse.com/docs/zh/operations/settings/settings#settings-http_native_compression_disable_checksumming_on_decompress)设置禁用服务器端校验。

如果在URL中指定了`compress=1`，服务会返回压缩的数据。 如果在URL中指定了`decompress=1`，服务会解压通过POST方法发送的数据。

您也可以选择使用[HTTP compression](https://en.wikipedia.org/wiki/HTTP_compression)。发送一个压缩的POST请求，附加请求头`Content-Encoding: compression_method`。为了使ClickHouse响应，您必须附加`Accept-Encoding: compression_method`。ClickHouse支持`gzip`，`br`和`deflate` [compression methods](https://en.wikipedia.org/wiki/HTTP_compression#Content-Encoding_tokens)。要启用HTTP压缩，必须使用ClickHouse[启用Http压缩](https://clickhouse.com/docs/zh/operations/settings/settings#settings-enable_http_compression)配置。您可以在[Http zlib压缩级别](https://clickhouse.com/docs/zh/interfaces/http#settings-http_zlib_compression_level)设置中为所有压缩方法配置数据压缩级别。
...
如果未指定用户名，则使用`default`。如果未指定密码，则使用空密码。 您还可以使用URL参数来指定处理单个查询或整个设置配置文件的任何设置。例子:`http://localhost:8123/?profile=web&max_rows_to_read=1000000000&query=SELECT+1`

更多信息，详见[设置](https://clickhouse.com/docs/zh/operations/settings#settings)部分。
...
您可以在`X-ClickHouse-Progress`响应头中收到查询进度的信息。为此，启用[Http Header携带进度](https://clickhouse.com/docs/zh/operations/settings/settings#settings-send_progress_in_http_headers)。示例：
...
可选的`query_id`参数可能当做query ID传入（或者任何字符串）。更多信息，详见[replace_running_query](https://clickhouse.com/docs/zh/operations/settings/settings)部分。

可选的`quota_key`参数可能当做quota key传入（或者任何字符串）。更多信息，详见[Quotas](https://clickhouse.com/docs/zh/operations/quotas#quotas)部分。

HTTP接口允许传入额外的数据（外部临时表）来查询。更多信息，详见[外部数据查询处理](https://clickhouse.com/docs/zh/engines/table-engines/special/external-data)部分。

## 响应缓冲[](https://clickhouse.com/docs/zh/interfaces/http#response-buffering)
...
### 查询参数[](https://clickhouse.com/docs/zh/interfaces/http#cli-queries-with-parameters)

您可以使用参数创建查询，并通过相应的HTTP请求参数为它们传递值。有关更多信息，请参见[CLI查询参数](https://clickhouse.com/docs/zh/interfaces/cli#cli-queries-with-parameters)。

### 示例[](https://clickhouse.com/docs/zh/interfaces/http#example)
...
## 特定的HTTP接口[](https://clickhouse.com/docs/zh/interfaces/http#predefined_http_interface)
...
- `handler` 包含主要的处理部分。现在`handler`可以配置`type`, `status`, `content_type`, `response_content`, `query`, `query_param_name`。 `type` 目前支持三种类型:[特定查询](https://clickhouse.com/docs/zh/interfaces/http#predefined_query_handler), [动态查询](https://clickhouse.com/docs/zh/interfaces/http#dynamic_query_handler), [static](https://clickhouse.com/docs/zh/interfaces/http#static).
...
### 特定查询[](https://clickhouse.com/docs/zh/interfaces/http#predefined_query_handler)
...
以下是定义的[max_threads](https://clickhouse.com/docs/zh/operations/settings/settings#settings-max_threads)和`max_final_threads`设置， 然后查询系统表以检查这些设置是否设置成功。
...
### 动态查询[](https://clickhouse.com/docs/zh/interfaces/http#dynamic_query_handler)
...
为了试验这个功能，示例定义了[max_threads](https://clickhouse.com/docs/zh/operations/settings/settings#settings-max_threads)和`max_final_threads`，`queries`设置是否成功的值。
...
### static[](https://clickhouse.com/docs/zh/interfaces/http#static)
