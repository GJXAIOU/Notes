...
连接完成后，如果你没有后续的动作，这个连接就处于空闲状态，你可以在 `show processlist` 命令中看到它。文本中这个图是 `show processlist` 的结果，其中的 Command 列显示为「Sleep」的这一行，就表示现在系统里面有一个空闲连接。（可以尝试开启两个 CMD 窗口，然后分别登陆 mysql，其中一台使用上述命令可以）
...
**但是大多数情况下建议不使用查询缓存，因为查询缓存往往弊大于利。**
