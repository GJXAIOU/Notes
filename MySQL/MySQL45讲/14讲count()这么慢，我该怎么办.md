...
InnoDB 针对 `count(*)` 做了一定的优化，因为 InnoDB 
...
示例如下：假设表 t 中现在有 10000 条记录，并有三个用户并行的会话。
...
因为**可重复读是 InnoDB 的默认事务隔离级别，在代码上就是通过多版本并发控制（MVCC）来实现**。每一行记录都要判断自己是否对这个会话可见，因此对于 `count(*)` 请求来说，InnoDB 只好把数据一行一行地读出依次判断，**可见的行才能够用于计算「基于这个查询」的表的总行数。**

> 备注： MVCC 可以回顾下第 3 讲《事务隔离：为什么你改了我还看不见？》和第 8 讲《事务到底是隔离的还是不隔离的？》中的相关内容。
