...
事务提交之后并不能马上删除 undo log 以及 undo log 所在的页，这是因为可能有 MVVC 使用（即其他事务需要通过 undo log 获取行记录之前的版本）。所以在事务提交时将 undo log 放在一个链表中，是否可以最终删除由purge线程判断。
...
右边改进方案：事务提交时，InnoDB 先进行 PREPARE 操作，将事务的 xid 写入，然后进行二进制日志写入。如果在 InnoDB 提交前，MySQL 宕机了，则MySQL 在重启后会先检查准备的 UXID 事务是否已经提交，如果没有则在存储引擎层再进行一次提交。
