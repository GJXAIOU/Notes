...
组成的层级包含：表空间(tablespace)，段(segment)，区(extent)，页(page) 。

### 表空间

InnoD B默认共享表空间为 data 目录下的 ibdata1，在开启`innodb_file_per_table`参数之后【每个表有独立的表空间】，独立表空间只保存表的**数据、索引、插入缓存**，而其它的例如：**回滚信息（undo），系统事务信息等**还在这个共享表空间里面的。
