...
将回调接口或注释与自定义的 `BeanPostProcessor` 实现结合使用是扩展 Spring IoC 容器的常用方法。Spring 的  `AutowiredAnnotationBeanPostProcessor` 就是一个例子 — `BeanPostProcessor` 实现，附带 Spring 发行版和 autowires 注解字段、setter 方法和任意配置方法。
