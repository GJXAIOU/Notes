...
> TODO：这里的上下文构造函数具体对应那一个函数？还是 public ClassPathXmlApplicationContext(String... configLocations) throws BeansException ？？？
...
然后可以使用 `getBean` 检索 bean 的实例。`ApplicationContext` 接口有一些其他方法用于检索 bean，但理想情况下，应用程序代码不应使用它们。==实际上，您的应用程序代码根本不应该调用 `getBean()` 方法，因此完全不依赖于 Spring API==。例如，Spring 与 web 框架的集成为各种 web 框架组件（如控制器和 JSF 托管 bean）提供了依赖项注入，允许您通过元数据（如自动装配注解）声明对特定 bean 的依赖项。==总结：使用 get-bean 方法，说明这个 bean 的执行且旅赖于通过 get-bean 方法获取的 bean 的==
...
>
> 同一个 XML 中不能有同 name 的 Bean，但是同一个容器中可以有（通过多个 XML 实现）。
...
> 
> 整体而言，Bean 的 id 和 name 属性类似于身份证号和人名。
> 
> > ```java
...
Spring IOC 容器实际上可以管理您希望它管理的任何类。它不仅限于管理真正的 JavaBeans。大多数 Spring 用户更喜欢实际的 JavaBean，它只有一个默认（无参数）构造函数，并根据容器中的属性建模适当的 setter 和 getter。您的容器中还可以有更多异国情调的非 bean 样式的类。例如，如果您需要使用一个完全不符合 JavaBean 规范的保持连接池，Spring 也可以管理它。
...
- `property` 和 `constructor-arg` 设置中的显式依赖项**始终覆盖自动装配**。**您不能自动装配简单属性**，如基本数据类型、`Strings` 和 `Classes`（以及此类简单属性的数组）。这一限制是故意造成的。
  ...
  > TODO：是一个 Bean 下面只要配置了一个 Property 就会替换所有的自动装配功能吗？
