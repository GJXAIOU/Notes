...
- 首先**程序计数器、虚拟机栈、本地方法栈都是随线程而生/灭**，大体上都是在编译期可知的（运行期会由 JIT 编译器进行优化因此它们在方法或者线程结束之后对应的内存空间就回收了。
...
- **一般使用 new 语句创建对象的时候消耗 12 个字节，其中引用在栈上占 4 个字节，空对象在堆中占 8 个字节。如果该语句所在方法执行结束之后，对应栈中的变量会马上进行回收，但是堆中的对象要等到 GC 来回收**。
    ...
    > 在Java中，我们并不能直接获取某个对象占用的栈和堆空间大小。因为Java的内存管理是由JVM（Java虚拟机）自动进行的，程序员通常不需要（也无法）直接操作内存。 
    >
    > 据此，要观察和监控Java的内存使用情况，通常会通过一些工具来实现。像是Java自带的命令行工具jmap、jstat等或者像VisualVM，MAT（Memory Analyzer Tool）等开源工具。 
    >
    > - jmap工具可以打印出一个Java进程中，所有对象的情况，包括每种对象的数量，占用空间大小等信息。例如，可以使用"jmap -histo:live [pid]"来打印对象信息。 
    > - VisualVM和MAT等图形界面工具则可以以更为直观的方式展示堆内存中的对象使用情况。同时，它们还提供了许多其他功能，例如分析内存泄露，查看线程状态等。 
    >
    > 需要注意的是，Java对象在堆中占用空间，而Java的栈空间通常是用来存放方法调用和局部变量的。对象引用可能会存放在栈空间，但是实际的对象数据是存放在堆空间的。 
    >
    > 另外，上述方法都只能得到近似的结果，因为在Java中，一个对象的大小还包括它引用的对象，以及JVM内部为了对象管理、垃圾回收等目的而添加的一些额外的数据。
...
- 相比使用「标记-清除」的 CMS，G1 使用的「复制」算法不会造成内存碎片;

- 对比 Parallel Scavenge（基于「复制」算法）、Parallel Old 收集器（基于 mark-compact-sweep），Parallel 会对整个区域做整理导致 GC 停顿会比较长，而 G1 只是特定地整理几个 region。
...
- G1 通过采用**将整个堆划分为相同大小的分区（Region）**策略来解决并行、串行和CMS收集器的碎片、暂停时间不可控等问题；
- 每个分区都可能是年轻代或老年代，但是在同一时刻只能属于某个代。年轻代、幸存区、老年代这些概念还存在，**但是成为逻辑上的概念**，这样方便复用之前分代框架的逻辑；
- 在物理上不需要连续，则带来了额外的好处-------有的分区内垃圾对象特别多，有的分区内垃圾对象很少，**G1 会优先回收垃圾对象特别多的分区**，这样可以花费较少的时间来回收这些分区的垃圾，这也就是 **G1 名字的由来，即首先收集垃圾最多的分区**。
- 依然是在新生代满了的时候，对整个新生代进行回收-----------整个新生代中的对象，要么被回收、要么晋升，至于新生代也采取分区机制的原因，则是因为这样跟老年代的策略统一，方便调整代的大小；
- G1 还是一种**带压缩的收集器**，在回收老年代的分区时，是将存活的对象从一个分区拷贝到另一个可用分区，这个拷贝的过程就实现了局部的压缩。

#### 2.收集集合（CSet：Collection Set）

一组可被回收的分区的集合。在 CSet 中存活的数据会在 GC 过程中被移动到另一个可用分区，CSet 中的分区可以来自 eden 空间、survivor 空间、 或者老年代。 Cset 在同一时刻可以拥有以上三种不同类型中的分区；

#### 3.已记忆集合（RSet：Remembered Set）
