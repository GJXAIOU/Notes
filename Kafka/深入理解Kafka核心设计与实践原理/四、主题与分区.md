...
可以通过 Kafka提供的 [kafka-topics.sh](https://link.zhihu.com/?target=http%3A//kafka-topics.sh) 脚本来执行这些操作。实质上是调用了 kafka.admin.TopicCommand 类来执行主题管理的操作。主题的管理还可以通过 KafkaAdminClient 的方式实现，甚至还可以通过直接操纵日志文件和ZooKeeper 节点来实现。

### （一）创建主题

如果 broker 端配置参数 `auto.create.topics.enable` 设置为 true（默认值就是 true，推荐设置为 false），那么当生产者向一个尚未创建的主题发送消息时或者当一个消费者开始从未知主题中读取消息时或者当任意一个客户端向未知主题发送元数据请求时，会自动创建一个分区数为 `num.partitions`（默认值为 1）、副本因子为 `default.replication.factor`（默认值为 1）的主题。
...
主题和分区是提供给上层用户的抽象，副本层（Log 层）是实际的物理存在，同一个分区的多个副本必须分布在不同的 broker 中，从而提供了有效的冗余。

> 4 个分区，2 个副本因子，broker 为 3 则按照 2/3/3 的分区副本个数分配给各个 broker；
>
> 如果是 3 个分区、2 个副本因子，broker 为 3 则按照 3/3/3 的分区副本个数分配给各个 broker，这样每个 broker 都拥有所有分区的一个副本。
...
```shell
D:\Kafka\kafka_2.13-3.2.0\bin\windows\kafka-topics.bat  --bootstrap-server localhost:9092,localhost:9093 --create --topic gjxaiou-same --replica-assignment 2:0,0:1,1:2,2:1
```

创建的效果：

```shell
D:\Kafka\kafka_2.13-3.2.0\bin\windows\kafka-topics.bat  --bootstrap-server localhost:9092,localhost:9093 --describe --topic gjxaiou,gjxaiou-same
